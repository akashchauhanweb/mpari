...
public final class LongSaturatedMathKt {
    ...
    private static final long m1471checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m1375isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m1445getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m1444getINFINITEUwyO8pc();
    }
    ...
    public static final long m1472saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM1389toLongimpl = Duration.m1389toLongimpl(j2, unit);
        if (((j - 1) | 1) == LongCompanionObject.MAX_VALUE) {
            return m1471checkInfiniteSumDefinedPjuGub4(j, j2, jM1389toLongimpl);
        }
        if ((1 | (jM1389toLongimpl - 1)) == LongCompanionObject.MAX_VALUE) {
            return m1473saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM1389toLongimpl;
        if (((j ^ j3) & (jM1389toLongimpl ^ j3)) >= 0) {
            return j3;
        }
        if (j < 0) {
    ...
    private static final long m1473saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM1346divUwyO8pc = Duration.m1346divUwyO8pc(j2, 2);
        long jM1389toLongimpl = Duration.m1389toLongimpl(jM1346divUwyO8pc, durationUnit);
        return (1 | (jM1389toLongimpl - 1)) == LongCompanionObject.MAX_VALUE ? jM1389toLongimpl : m1472saturatingAddNuflL3o(m1472saturatingAddNuflL3o(j, durationUnit, jM1346divUwyO8pc), durationUnit, Duration.m1378minusLRDsOJo(j2, jM1346divUwyO8pc));
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        return (1 | (j2 - 1)) == LongCompanionObject.MAX_VALUE ? Duration.m1396unaryMinusUwyO8pc(infinityOfSign(j2)) : saturatingFiniteDiff(j, j2, unit);
    }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (durationUnit.compareTo(durationUnit2) >= 0) {
            return Duration.m1396unaryMinusUwyO8pc(infinityOfSign(j3));
        }
        ...
        return Duration.m1379plusLRDsOJo(DurationKt.toDuration(j4, durationUnit2), DurationKt.toDuration(j5, durationUnit));
    }

    public static final long saturatingOriginsDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        return ((j2 - 1) | 1) == LongCompanionObject.MAX_VALUE ? j == j2 ? Duration.INSTANCE.m1446getZEROUwyO8pc() : Duration.m1396unaryMinusUwyO8pc(infinityOfSign(j2)) : (1 | (j - 1)) == LongCompanionObject.MAX_VALUE ? infinityOfSign(j) : saturatingFiniteDiff(j, j2, unit);
    }
}
