...
public abstract class kz2 {
    ...
    public kz2(byte[] bArr, int i) throws InvalidKeyException {
        if (bArr.length != 32) {
    ...
    public final void f(byte[] bArr, ByteBuffer byteBuffer, ByteBuffer byteBuffer2) throws GeneralSecurityException {
        ...
        for (int i3 = 0; i3 < i2; i3++) {
            ...
            if (i3 == i) {
                y80.c(byteBuffer, byteBuffer2, byteBufferA, iRemaining % 64);
            } else {
                y80.c(byteBuffer, byteBuffer2, byteBufferA, 64);
            }
        }
    }
}
