...
final class ULongProgressionIterator implements Iterator<ULong>, KMappedMarker {
    ...
    public /* bridge */ /* synthetic */ ULong next() {
        return ULong.m174boximpl(m1251nextsVKNKU());
    }

    /* JADX INFO: renamed from: next-s-VKNKU, reason: not valid java name */
    public long m1251nextsVKNKU() {
        long j = this.next;
        if (j != this.finalElement) {
            this.next = ULong.m180constructorimpl(this.step + j);
        } else {
            if (!this.hasNext) {
    ...
    private ULongProgressionIterator(long j, long j2, long j3) {
        ...
        this.hasNext = z;
        this.step = ULong.m180constructorimpl(j3);
